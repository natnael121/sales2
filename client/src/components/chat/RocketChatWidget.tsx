...
export const RocketChatWidget: React.FC = () => {
  ...
  useEffect(() => {
    const initializeRocketChat = async () => {
      ...
      try {
        ...
        // Use the new backend-based connection approach
        ...
        if (success) {
          ...
          console.warn('Rocket.Chat backend unavailable, falling back to Firebase chat');
          setConnected(false);
        }
      } catch (error) {
        console.error('Failed to initialize Rocket.Chat:', error);
        console.warn('Falling back to Firebase chat due to Rocket.Chat error');
        setConnected(false);
      } finally {
        setLoading(false);
      }
  ...
  const loadRooms = async () => {
    try {
      ...
      const chatRooms: ChatRoom[] = rocketRooms.map(room => ({
        ...
        unreadCount: 0, // Would need to implement unread counting
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40" data-testid="rocket-chat-widget">
      <Button
          ...
          <CardHeader className="p-4 border-b">
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg">
                {connected ? "Team Chat" : "Connecting..."}
              </CardTitle>
            ...
            {connected && (
              <div className="flex items-center space-x-1 text-sm text-muted-foreground">
                ...
                <span>Connected to Rocket.Chat</span>
              </div>
            )}
          </CardHeader>
            ...
            {!connected && (
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center text-muted-foreground">
                  {loading ? "Connecting to chat..." : "Chat unavailable"}
                </div>
              </div>
            )}
